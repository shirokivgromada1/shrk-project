            ...
            {description.children.map((article: any, articleIndex: number) => (
              <article.type key={"article" + articleIndex}>
                {article.children.map((piece: any, pieceIndex: number) => {
                  if (piece.type && piece.type.startsWith("h"))
                    return piece.children.map(
                      (text: any, textIndex: number) => (
                        <piece.type
                          style={{
                            fontWeight: piece.bold && "bold",
                            fontStyle: piece.italic && "italic",
                          }}
                          key={"descriptionInfo" + textIndex}
                        >
                          {text.text}
                        </piece.type>
                      )
                      ...
                      <a key={"link" + pieceIndex} href={piece.url}>
                        {piece.children.map(
                          (linkTitle: any, linkTitleIndex: number) => (
                            <span
                              style={{
                                fontWeight: linkTitle.bold && "bold",
                                fontStyle: linkTitle.italic && "italic",
                              }}
                              key={"linkTitle" + linkTitleIndex}
                            >
                              {linkTitle.text}
                            </span>
                          )
