import { Button, Step, StepContent, StepLabel, TextField } from "@mui/material";
...
export const Phone = ({
  ...
  useEffect(() => {
    if (phoneNumber === "+380") setEditMode(false);
  }, [phoneNumber]);

  const handleClick = () => {
    axios
      .post(
        `${process.env.NEXT_PUBLIC_APP_BASE_URL}${AUTH_ENDPOINTS.REGISTER}/`,
        {
          phoneNumber,
        }
      )
      ...
      <StepContent className={styles.stepper__step_content}>
        <div className={styles.stepper__step_content_wrapper}>
          ...
          <div>
            <Button
              variant="contained"
              disabled={!isEnabled || isActiveTimer}
              onClick={handleClick}
            >
